...
Get API Key"](https://developers.google.com/maps/documentation/geocoding/get-api-key) 웹페이지를 참조하여 API키를 받을 수도 있고, `?register_google`을 참조하여 인증작업을 시작하면 되는데 모든 구글 지리정보 API사용이 유료화되어 아래 코드를 남겨두는 것으로 갈음하고자한다.

**지금은 더 이상 아래 코드가 인증이 되지 않아 동작하지 않습니다.**
...
`C:\Program Files\R\R-3.5.2\etc` 디렉토리 `Rprofile.site` 파일에 [카카오 개발자센터](https://developers.kakao.com/) 앱개발에 사용할 API 키를 `DAUM_MAP_API_KEY` 변수에 저장시킨다.
재미있는 것은 `Authorization`에서 `"KakaoAK "`를 꼭 앞에 붙여야함으로 `paste0()` 함수로 결합시켜 던진다.  
